...
fn test_sub() {
    ...
    assert_eq!(SValue::INFINITY, SValue::NAN - SValue::NEG_INFINITY);
    assert_eq!(SValue::NEG_INFINITY, SValue::NAN - SValue::INFINITY);
    ...
    for zero in int2reprsZ(0) {
        ...
        assert_eq!(SValue::NEG_INFINITY, zero.clone() - SValue::INFINITY);
        assert_eq!(SValue::NEG_INFINITY, SValue::NEG_INFINITY - zero.clone());
        assert_eq!(SValue::INFINITY, zero - SValue::NEG_INFINITY);
    }
    for (zero1, zero2) in ints2reprsZ(0, 0) {
        assert_eq!(SValue::Int(0), zero1 - zero2);
    }
    assert_eq!(SValue::Float(2.1), SValue::Float(4.2) - SValue::Float(2.1));

    assert_eq!(SValue::NAN, SValue::INFINITY - SValue::INFINITY);
    assert_eq!(SValue::INFINITY, SValue::INFINITY - SValue::NEG_INFINITY);
    assert_eq!(SValue::INFINITY, SValue::INFINITY - SValue::Float(4.2));
    assert_eq!(SValue::INFINITY, SValue::INFINITY - SValue::Float(-4.2));

    assert_eq!(SValue::NEG_INFINITY, SValue::Float(4.2) - SValue::INFINITY);
    assert_eq!(SValue::NEG_INFINITY, SValue::Float(-4.2) - SValue::INFINITY);
    assert_eq!(
        SValue::NEG_INFINITY,
        SValue::NEG_INFINITY - SValue::INFINITY
    );
    assert_eq!(SValue::NAN, SValue::NEG_INFINITY - SValue::NEG_INFINITY);

    for finite in [4, -4] {
        for num in int2reprsZ(finite) {
            assert_eq!(SValue::INFINITY, num.clone() - SValue::NEG_INFINITY);
    ...
    assert_eq!(SValue::Int(10), SValue::Int(12) - SValue::Int(2));
    assert_eq!(SValue::Int(10), SValue::Int(12) - SValue::Text("2".into()));
    assert_eq!(SValue::Float(2.1), SValue::Float(4.2) - SValue::Float(2.1));
